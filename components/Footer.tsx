...
export default function Footer() {
  ...
  const contactClass = isNeobrutalist
    ? "font-black text-black bg-white border-2 border-black px-3.5 py-1.5 shadow-[3px_3px_0px_#000000] rounded-none hover:bg-[#FFE600] active:translate-x-[2px] active:translate-y-[2px] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
    : "text-[var(--text-muted)] hover:text-[var(--text-primary)] group focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]";
      ...
      <div className="mb-10">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-3 text-[13.5px]">
          <a
            ...
            className={`inline-flex items-center gap-2 transition-all ${contactClass}`}
          >
            <Mail className="w-4 h-4 opacity-80" aria-hidden="true" />
            <span>aerol.balayon@gmail.com</span>
            <span className={isNeobrutalist ? "font-black" : "text-[var(--text-dim)]"} aria-hidden="true">↗</span>
          </a>
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 transition-all ${contactClass}`}
            aria-label="Aedwon on GitHub, opens in a new tab"
          >
            <GitHubIcon />
            <span>/Aedwon</span>
            <span className={isNeobrutalist ? "font-black" : "text-[var(--text-dim)]"} aria-hidden="true">↗</span>
          ...
          <span
            className={`inline-flex items-center gap-2 ${
              isNeobrutalist
                ? "font-black text-black bg-white border-2 border-black px-3.5 py-1.5 shadow-[3px_3px_0px_#000000] rounded-none"
                : "text-[var(--text-muted)]"
            }`}
            aria-label="Discord handle: at aedwon"
          >
            <DiscordIcon />
            <span>@aedwon</span>
          </span>
          ...
          <a
            ...
            rel="noopener noreferrer"
            className={`inline-flex items-center gap-2 transition-all ${contactClass}`}
            aria-label="Aedwon on LinkedIn, opens in a new tab"
          >
            <LinkedInIcon />
            <span>/in/aedwon</span>
            <span className={isNeobrutalist ? "font-black" : "text-[var(--text-dim)]"} aria-hidden="true">↗</span>
...
function GitHubIcon() {
  return (
    <svg className="w-4 h-4 fill-current opacity-80" viewBox="0 0 24 24" aria-hidden="true">
